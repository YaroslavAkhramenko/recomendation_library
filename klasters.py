...
def get_the_best(index):
  ...
  if not index in users_pos_pca.index:
    return 'Death Note'
  ...
  for i in rec[label]:
    ...
    if rating.loc[rating['anime_id'] == ANIME_ID].loc[rating['user_id'] == index].empty:
      return i
...
def add(ANIME_NAME, USER_ID, RATING):
    ...
    for i in anime3.loc[anime3['anime_id']==anime_id].drop('anime_id', axis=1):
        if anime3.loc[anime3['anime_id']==anime_id][i].values!=0:
    ...
    global rating
    rating = rating.append({'user_id': int(USER_ID), 'anime_id': int(anime_id),
                            'rating': int(RATING)}, ignore_index=True)
...
# print(rating)
FIRST = get_the_best(5444444)
print(FIRST + ' was')
add(FIRST, 5444444, 10)
print(get_the_best(5444444))
